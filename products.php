...
{
  $search=$_POST['name'];
  $sql="SELECT * FROM products WHERE LOCATE('$search',product_name)>0 OR  LOCATE('$search',details)>0 OR  LOCATE('$search',category)>0 ";
  ...
  if ($productCount > 0) {
    while($row = mysqli_fetch_array($result)){ 
      ...
      //Retrieving the products
       $dynamicList.= '<div class="col-md-4" style="margin-top:20px; margin-bottom:20px; "><div class="card" style="box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2); max-width: 300px; margin: auto; text-align: center; font-family: arial; background:white;; min-height:420px;">
...
if ($productCount > 0) {
	while($row = mysqli_fetch_array($result)){ 
       ...
       $dynamicList.= '<div class="col-md-4" style="margin-top:20px; margin-bottom:20px; "><div class="card" style="max-width: 300px; margin: auto; text-align: center; font-family: arial; background:white; min-height: 420px;">
         <img src="inventory_images/' . $product_name . '.jpg" " alt="'.$product_name.'" style="width:100%; height:250px;" class="img-rounded">
         <h3>'. $product_name.' </h3>
