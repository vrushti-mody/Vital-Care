...
{
  $search=$_POST['name'];
  $sql="SELECT * FROM products WHERE (LOCATE('$search',product_name)>0 OR  LOCATE('$search',details)>0 OR  LOCATE('$search',category)>0 AND category='HEALTH & FITNESS') ";
...
$sql = "SELECT * FROM products WHERE category='HEALTH & FITNESS'";
$result = mysqli_query($mysqli,$sql);
